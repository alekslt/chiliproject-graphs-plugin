...
class TargetVersionGraphHook < Redmine::Hook::ViewListener
  def view_versions_show_bottom(context = { })
      ...
      output << tag("embed", :width => "100%", :height => 500, :type => "text/html", :src => url_for(:controller => 'graphs', :action => 'target_version_status_graphjs', :id => context[:version]) << "?size=small" )
      output << "</a>\n"
      output << "</fieldset>\n"
      ...
      output << tag("embed", :width => "100%", :height => 500, :type => "text/html", :src => url_for(:controller => 'graphs', :action => 'target_version_status_graphjs', :id => context[:version]) << "?size=small&lastdays=15" )
      output << "</a>\n"
      output << "</fieldset>\n"
